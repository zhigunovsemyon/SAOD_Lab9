...
/*
В некоторой древовидной бинарной структуре опытным путем измеряется частота
обращения к каждому из элементов. Для этого с любым из элементов связан
счетчик обращений. По прошествии определенного периода времени дерево
реорганизуется. Для этого оно просматривается и с помощью соответствующей
программы строится новое дерево, в которое ключи включаются в порядке
убывания счетчиков частот обращения. Напишите программы, выполняющие такую
реорганизацию. Будет ли средняя длина пути в новом дереве равна, больше или
...
{
	while (--count)
		TreeBelongs(t, arr + ((size_t)rand() % arrsize));
}
